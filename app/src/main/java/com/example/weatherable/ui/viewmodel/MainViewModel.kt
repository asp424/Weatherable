        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (repository.getAllPressure()!!.isNotEmpty())
            {
                ...
                repository.getAllPressure()?.forEach {
                    listPoints.add(Offset(x = ((((it.id.toFloat()) - oX!!)/10000) + 110f)/14 ,
                        y = -(it.pressure?.toFloat()!! - oY)*10 + 180f))
                    if (listPoints.size == repository.getAllPressure()?.size)
                        _pointsList.value = listPoints
                    Log.d("My", listPoints.toString())
                   repository.getAllPressure()!!.forEach {a ->
                       Log.d("My",a.pressure.toString())
                   }
                }
            }
        }
